  ...
  if (options.production && !options.disableWalk) {
    var walker = function () {
      var walker   = walk.walk(options.viewsDir)
        , results  = []
        , regexCDN = options.viewEngine === 'handlebars' ? /\{\{CDN\s((\([^)]+\)|[^)])+?)}}/ig : /CDN\(((\([^)]+\)|[^)])+)\)/ig;
      walker.on('file', function(root, stat, next) {
        var validExts = options.extensions || ['.jade', '.ejs', '.handlebars'];
      ...
      walker.on('end', function() {
        ...
        for (var i = 0; i < results.length; i += 1) {
          ...
          if(options.viewEngine === 'handlebars') {
            obj.attributes = [];
            results[i].replace(/([a-zA-Z_:][-a-zA-Z0-9_:.]*)(?:\s*=\s*(?:(?:"((?:\\.|[^"])*)")|(?:'((?:\\.|[^'])*)')|([^>\s]+)))?/g, function (match, name) {
            ...
            console.log('AFTER ASSETS %j', results[i]);
            // TODO: Add Support for concatenating files using an array of asset names
            obj.assets = results[i].split(' ')[0];
            console.log('ASSETS', obj.assets);
            results[i] = obj;
            console.log('RESULTS I RESULT %j', results[i]);
          } else {
            // Convert all apostrophes
            results[i] = results[i].replace(/\'/g, '"');
